...
+ (UIImage *)editedImageFromImage:(UIImage *)image andFrame:(CGRect)frameRect andImageRect:(CGRect)imageRect andImageViewWidth:(CGFloat)width andImageViewHeight:(CGFloat)height
{
    ...
    UIImage *croppedImage = [UIImage imageWithCGImage:imageReference scale:image.scale orientation:UIImageOrientationUp]; //Hardcoded orientation metadata because we have rotated the actual image.
...
{
    CGSize size = initImage.size;
    UIGraphicsBeginImageContext(CGSizeMake(size.height, size.width));
    [[UIImage imageWithCGImage:[initImage CGImage] scale:1.0 orientation:orientation] drawInRect:CGRectMake(0,0,size.width ,size.height)];
