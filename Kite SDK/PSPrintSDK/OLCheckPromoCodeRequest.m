...
- (void)checkPromoCode:(NSString *)promoCode withOrder:(OLPrintOrder *)order andCompletionHandler:(OLCheckPromoCodeCompletionHandler)handler {
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/%@/promo_code/check?code=%@&templates=%@&currency=%@", [OLKitePrintSDK apiEndpoint], [OLKitePrintSDK apiVersion], urlencode(promoCode), templateCostBreakdown, order.currencyCode]];
