...
while($folderLocations =~ /iPhone 4s \((.{8}-.*?)\)/g) { # Use regex to loop through each iPhone 4s device found in $folderLocations. Insert the permissions in the database of each.
    print "folderLocations <1>: $1\n";  # debug logs
    `xcrun simctl boot $1; sleep 10; xcrun simctl shutdown $1`;
