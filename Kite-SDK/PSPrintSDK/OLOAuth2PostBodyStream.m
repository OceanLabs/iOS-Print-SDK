...
- (id)initWithParameters:(NSDictionary *)postParameters;
{
    ...
    if (self) {
        ...
        boundary = [[NSString alloc] initWithFormat:@"------------OL-oauth2%d", rand()];
        numBytesTotal = 0;
        streamIndex = 0;
    ...
    for (NSString *key in parameters) {
        ...
        if ([value isKindOfClass:[NSArray class]]) {
            ...
            for (id content in contentArray) {
                OLOAuth2PostBodyPart *part = [[OLOAuth2PostBodyPart alloc] initWithName:key content:content];
                [parts addObject:part];
            }
        } else {
            OLOAuth2PostBodyPart *part = [[OLOAuth2PostBodyPart alloc] initWithName:key content:value];
            [parts addObject:part];
        }
    }
    return parts;
...
- (NSArray *)streamsForParameters:(NSDictionary *)parameters contentLength:(unsigned long long *)contentLength;
{
    ...
    for (OLOAuth2PostBodyPart *part in parts) {
        @autoreleasepool {
