...
NSString * const OLKeyChainStoreErrorDomain = @"ly.kite.olkeychainstore";
static NSString *_defaultService;

@interface OLKeyChainStore ()

@end

@implementation OLKeyChainStore
...
+ (OLKeyChainStore *)keyChainStoreWithService:(NSString *)service
{
    return [[self alloc] initWithService:service accessGroup:nil];
}

+ (OLKeyChainStore *)keyChainStoreWithService:(NSString *)service accessGroup:(NSString *)accessGroup
{
...
{
    return [[self alloc] initWithServer:server protocolType:protocolType authenticationType:OLKeyChainStoreAuthenticationTypeDefault];
}
...
- (instancetype)initWithService:(NSString *)service accessGroup:(NSString *)accessGroup
{
    self = [super init];
    if (self) {
        _itemClass = OLKeyChainStoreItemClassGenericPassword;
...
{
    return [self initWithServer:server protocolType:protocolType authenticationType:OLKeyChainStoreAuthenticationTypeDefault];
}
...
- (instancetype)initWithServer:(NSURL *)server protocolType:(OLKeyChainStoreProtocolType)protocolType authenticationType:(OLKeyChainStoreAuthenticationType)authenticationType
{
    self = [super init];
    if (self) {
        _itemClass = OLKeyChainStoreItemClassInternetPassword;
...
- (void)commonInit
{
    _accessibility = OLKeyChainStoreAccessibilityAfterFirstUnlock;
}
...
{
    if (!key) {
        NSError *e = [self argumentError:@"the key must not to be nil"];
        if (error) {
            *error = e;
        }
        return nil;
    ...
    OLKeyChainStore *keychain = [OLKeyChainStore keyChainStoreWithService:service accessGroup:accessGroup];
...
+ (BOOL)setString:(NSString *)value forKey:(NSString *)key service:(NSString *)service accessGroup:(NSString *)accessGroup genericAttribute:(id)genericAttribute error:(NSError * __autoreleasing *)error
{
    ...
    NSError *e = [self conversionError:@"failed to convert string to data"];
    if (error) {
        *error = e;
    }
    return NO;
...
- (NSString *)stringForKey:(id)key error:(NSError *__autoreleasing *)error
{
    ...
    if (data) {
        ...
        NSError *e = [self.class conversionError:@"failed to convert data to string"];
        if (error) {
            *error = e;
        }
        return nil;
...
- (BOOL)setString:(NSString *)string forKey:(NSString *)key genericAttribute:(id)genericAttribute label:(NSString *)label comment:(NSString *)comment error:(NSError *__autoreleasing *)error
{
    ...
    NSError *e = [self.class conversionError:@"failed to convert string to data"];
    if (error) {
        *error = e;
    }
    return NO;
...
- (NSData *)dataForKey:(NSString *)key error:(NSError *__autoreleasing *)error
{
    ...
    if (status == errSecSuccess) {
        ...
        if (data) {
            ...
            NSError *e = [self.class unexpectedError:@"Unexpected error has occurred."];
            if (error) {
                *error = e;
            }
            return nil;
...
{
    if (!key) {
        NSError *e = [self.class argumentError:@"the key must not to be nil"];
        if (error) {
            *error = e;
        }
        return NO;
...
+ (NSArray OL_KEY_TYPE *)allKeysWithItemClass:(OLKeyChainStoreItemClass)itemClass
{
    CFTypeRef itemClassObject = kSecClassGenericPassword;
    if (itemClass == OLKeyChainStoreItemClassGenericPassword) {
        itemClassObject = kSecClassGenericPassword;
    } else if (itemClass == OLKeyChainStoreItemClassInternetPassword) {
...
+ (NSArray *)allItemsWithItemClass:(OLKeyChainStoreItemClass)itemClass
{
    CFTypeRef itemClassObject = kSecClassGenericPassword;
    if (itemClass == OLKeyChainStoreItemClassGenericPassword) {
        itemClassObject = kSecClassGenericPassword;
...
- (void)sharedPasswordWithCompletion:(void (^)(NSString *account, NSString *password, NSError *error))completion
{
    ...
    if (domain.length > 0) {
        ...
        NSError *error = [self.class argumentError:@"the server property must not to be nil, should use 'keyChainStoreWithServer:protocolType:' initializer to instantiate keychain store"];
        if (completion) {
            completion(nil, nil, error);
        }
    }
...
+ (void)requestSharedWebCredentialWithCompletion:(void (^)(NSArray OL_CREDENTIAL_TYPE *credentials, NSError *error))completion
{
    [self requestSharedWebCredentialForDomain:nil account:nil completion:completion];
}

+ (void)requestSharedWebCredentialForDomain:(NSString *)domain account:(NSString *)account completion:(void (^)(NSArray OL_CREDENTIAL_TYPE *credentials, NSError *error))completion
{
...
- (NSMutableDictionary *)attributesWithKey:(NSString *)key value:(NSData *)value error:(NSError *__autoreleasing *)error
{
    ...
    if (_authenticationPolicy && accessibilityObject) {
        ...
        if (floor(NSFoundationVersionNumber) <= floor(iOS_7_1_or_10_9_2) && _accessibility == OLKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly) {
#if TARGET_OS_IOS
            NSLog(@"%@", @"Unavailable 'OLKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly' attribute on iOS versions prior to 8.0.");
#else
            NSLog(@"%@", @"Unavailable 'OLKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly' attribute on OS X versions prior to 10.10.");
#endif
        } else {
...
{
    switch (_itemClass) {
        case OLKeyChainStoreItemClassGenericPassword:
            return kSecClassGenericPassword;
        case OLKeyChainStoreItemClassInternetPassword:
...
- (CFTypeRef)protocolTypeObject
{
    switch (_protocolType) {
        case OLKeyChainStoreProtocolTypeFTP:
            return kSecAttrProtocolFTP;
        case OLKeyChainStoreProtocolTypeFTPAccount:
            return kSecAttrProtocolFTPAccount;
        case OLKeyChainStoreProtocolTypeHTTP:
            return kSecAttrProtocolHTTP;
        case OLKeyChainStoreProtocolTypeIRC:
            return kSecAttrProtocolIRC;
        case OLKeyChainStoreProtocolTypeNNTP:
            return kSecAttrProtocolNNTP;
        case OLKeyChainStoreProtocolTypePOP3:
            return kSecAttrProtocolPOP3;
        case OLKeyChainStoreProtocolTypeSMTP:
            return kSecAttrProtocolSMTP;
        case OLKeyChainStoreProtocolTypeSOCKS:
            return kSecAttrProtocolSOCKS;
        case OLKeyChainStoreProtocolTypeIMAP:
            return kSecAttrProtocolIMAP;
        case OLKeyChainStoreProtocolTypeLDAP:
            return kSecAttrProtocolLDAP;
        case OLKeyChainStoreProtocolTypeAppleTalk:
            return kSecAttrProtocolAppleTalk;
        case OLKeyChainStoreProtocolTypeAFP:
            return kSecAttrProtocolAFP;
        case OLKeyChainStoreProtocolTypeTelnet:
            return kSecAttrProtocolTelnet;
        case OLKeyChainStoreProtocolTypeSSH:
            return kSecAttrProtocolSSH;
        case OLKeyChainStoreProtocolTypeFTPS:
            return kSecAttrProtocolFTPS;
        case OLKeyChainStoreProtocolTypeHTTPS:
            return kSecAttrProtocolHTTPS;
        case OLKeyChainStoreProtocolTypeHTTPProxy:
            return kSecAttrProtocolHTTPProxy;
        case OLKeyChainStoreProtocolTypeHTTPSProxy:
            return kSecAttrProtocolHTTPSProxy;
        case OLKeyChainStoreProtocolTypeFTPProxy:
            return kSecAttrProtocolFTPProxy;
        case OLKeyChainStoreProtocolTypeSMB:
            return kSecAttrProtocolSMB;
        case OLKeyChainStoreProtocolTypeRTSP:
            return kSecAttrProtocolRTSP;
        case OLKeyChainStoreProtocolTypeRTSPProxy:
            return kSecAttrProtocolRTSPProxy;
        case OLKeyChainStoreProtocolTypeDAAP:
            return kSecAttrProtocolDAAP;
        case OLKeyChainStoreProtocolTypeEPPC:
            return kSecAttrProtocolEPPC;
        case OLKeyChainStoreProtocolTypeNNTPS:
            return kSecAttrProtocolNNTPS;
        case OLKeyChainStoreProtocolTypeLDAPS:
            return kSecAttrProtocolLDAPS;
        case OLKeyChainStoreProtocolTypeTelnetS:
            return kSecAttrProtocolTelnetS;
        case OLKeyChainStoreProtocolTypeIRCS:
            return kSecAttrProtocolIRCS;
        case OLKeyChainStoreProtocolTypePOP3S:
...
- (CFTypeRef)authenticationTypeObject
{
    switch (_authenticationType) {
        case OLKeyChainStoreAuthenticationTypeNTLM:
            return kSecAttrAuthenticationTypeNTLM;
        case OLKeyChainStoreAuthenticationTypeMSN:
            return kSecAttrAuthenticationTypeMSN;
        case OLKeyChainStoreAuthenticationTypeDPA:
            return kSecAttrAuthenticationTypeDPA;
        case OLKeyChainStoreAuthenticationTypeRPA:
            return kSecAttrAuthenticationTypeRPA;
        case OLKeyChainStoreAuthenticationTypeHTTPBasic:
            return kSecAttrAuthenticationTypeHTTPBasic;
        case OLKeyChainStoreAuthenticationTypeHTTPDigest:
            return kSecAttrAuthenticationTypeHTTPDigest;
        case OLKeyChainStoreAuthenticationTypeHTMLForm:
            return kSecAttrAuthenticationTypeHTMLForm;
        case OLKeyChainStoreAuthenticationTypeDefault:
...
- (CFTypeRef)accessibilityObject
{
    switch (_accessibility) {
        case OLKeyChainStoreAccessibilityWhenUnlocked:
            return kSecAttrAccessibleWhenUnlocked;
        case OLKeyChainStoreAccessibilityAfterFirstUnlock:
            return kSecAttrAccessibleAfterFirstUnlock;
        case OLKeyChainStoreAccessibilityAlways:
            return kSecAttrAccessibleAlways;
        case OLKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly:
            return kSecAttrAccessibleWhenPasscodeSetThisDeviceOnly;
        case OLKeyChainStoreAccessibilityWhenUnlockedThisDeviceOnly:
            return kSecAttrAccessibleWhenUnlockedThisDeviceOnly;
        case OLKeyChainStoreAccessibilityAfterFirstUnlockThisDeviceOnly:
            return kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly;
        case OLKeyChainStoreAccessibilityAlwaysThisDeviceOnly:
...
{
    NSError *error = [NSError errorWithDomain:OLKeyChainStoreErrorDomain code:OLKeyChainStoreErrorInvalidArguments userInfo:@{NSLocalizedDescriptionKey: message}];
...
{
    NSError *error = [NSError errorWithDomain:OLKeyChainStoreErrorDomain code:-67594 userInfo:@{NSLocalizedDescriptionKey: message}];
    NSLog(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}
...
#endif
    NSError *error = [NSError errorWithDomain:OLKeyChainStoreErrorDomain code:status userInfo:@{NSLocalizedDescriptionKey: message}];
...
{
    NSError *error = [NSError errorWithDomain:OLKeyChainStoreErrorDomain code:-99999 userInfo:@{NSLocalizedDescriptionKey: message}];
    NSLog(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}
