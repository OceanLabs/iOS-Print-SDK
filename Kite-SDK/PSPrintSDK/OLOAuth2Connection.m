...
@interface OLOAuth2Client (Private)
- (void)removeConnectionFromWaitingQueue:(OLOAuth2Connection *)connection;
@end


NSString * const OLOAuth2ConnectionDidStartNotification = @"OLOAuth2ConnectionDidStartNotification";
NSString * const OLOAuth2ConnectionDidEndNotification = @"OLOAuth2ConnectionDidEndNotification";


@interface OLOAuth2Connection ()
...
- (BOOL)trustsAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge
                        ...
                        withTrustMode:(OLOAuth2TrustMode)trustMode;
...
@property (nonatomic, unsafe_unretained, readonly) id<OLOAuth2TrustDelegate> trustDelegate;


@end


@implementation OLOAuth2Connection

#pragma mark Lifecycle

-  (id)initWithRequest:(NSMutableURLRequest *)aRequest
     requestParameters:(NSDictionary *)someRequestParameters
           oauthClient:(OLOAuth2Client *)aClient
sendingProgressHandler:(OLOAuth2ConnectionSendingProgressHandler)aSendingProgressHandler
       responseHandler:(OLOAuth2ConnectionResponseHandler)aResponseHandler;
{
...
- (id)initWithRequest:(NSMutableURLRequest *)aRequest
    requestParameters:(NSDictionary *)someRequestParameters
          oauthClient:(OLOAuth2Client *)aClient
             delegate:(NSObject<OLOAuth2ConnectionDelegate> *)aDelegate;
{
    self = [super init];
    if (self) {
...
- (void)dealloc;
{
    if (sendConnectionDidEndNotification) [[NSNotificationCenter defaultCenter] postNotificationName:OLOAuth2ConnectionDidEndNotification object:self];
    sendConnectionDidEndNotification = NO;
...
- (id<OLOAuth2TrustDelegate>)trustDelegate;
{
    // if a client is set and implemnts the trustModeForHostname: it is preferred
    // in making trust desicions.
    // The second choice
    if (client && [client.delegate conformsToProtocol:@protocol(OLOAuth2TrustDelegate)]) {
        return (id<OLOAuth2TrustDelegate>)client.delegate;
    } else if ([delegate conformsToProtocol:@protocol(OLOAuth2TrustDelegate)]) {
        return (id<OLOAuth2TrustDelegate>)delegate;
    }
    return nil;
}
...
{
    return [NSString stringWithFormat:@"OLOAuth2Connection <%@>", request.URL];
}
...
- (NSURLConnection *)createConnection;
{
    ...
    if (!sendConnectionDidEndNotification) [[NSNotificationCenter defaultCenter] postNotificationName:OLOAuth2ConnectionDidStartNotification object:self];
...
- (void)applyParameters:(NSDictionary *)parameters onRequest:(NSMutableURLRequest *)aRequest;
{
    ...
    if ([httpMethod caseInsensitiveCompare:@"POST"] != NSOrderedSame
        ...
        aRequest.URL = [aRequest.URL oloauth2_URLByAddingParameters:parameters];
        ...
        if (!contentType || [contentType isEqualToString:@"multipart/form-data"]) {
            ...
            contentType = [NSString stringWithFormat:@"multipart/form-data; boundary=%@",[(OLOAuth2PostBodyStream *)postBodyStream boundary]];
            NSString *contentLength = [NSString stringWithFormat:@"%lld", [(OLOAuth2PostBodyStream *)postBodyStream length]];
            ...
            NSString *query = [[aRequest.URL oloauth2_URLByAddingParameters:parameters] query];
...
- (BOOL)trustsAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge
                        ...
                        withTrustMode:(OLOAuth2TrustMode)trustMode;
{
    if (trustMode & OLOAuth2TrustModeAnyCertificate) {
        return YES;
    }
    
    if (trustMode & OLOAuth2TrustModeSystem) {
    ...
    if (trustMode & OLOAuth2TrustModeSpecificCertificate) {
        NSAssert([self.trustDelegate respondsToSelector:@selector(connection:trustedCertificatesForHostname:)],
                 @"For OLOAuth2TrustModeSpecificCertificate the delegate needs to implement oauthConnection:trustedCertificatesDERDataForHostname:");
    ...
    if (checkResult == kSecTrustResultProceed || checkResult == kSecTrustResultUnspecified) {
        ...
        if (errGetTrustResult == noErr) {
            ...
            for (id certificate in certificates) {
                ...
                NSString *certificateChecksum = [(__bridge_transfer NSData *)certData ol_SHA1Hexdigest];
                NSString *anchorCertChecksum = [derCertData ol_SHA1Hexdigest];
...
- (void)stream:(OLOAuth2PostBodyStream *)stream didSendBytes:(unsigned long long)deliveredBytes ofTotal:(unsigned long long)totalBytes;
{
...
{
    return [[OLOAuth2PostBodyStream alloc] initWithParameters:requestParameters];
}
...
- (void)connection:(NSURLConnection *)connection didReceiveAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge;
{
    if ([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
        ...
        OLOAuth2TrustMode effectiveTrustMode = OLOAuth2TrustModeSystem;
