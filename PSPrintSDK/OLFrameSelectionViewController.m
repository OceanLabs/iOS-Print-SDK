...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if ([segue.identifier isEqualToString:@"Selected2x2FrameStyleSegue"]) {
        for (OLProduct *product in products){
            if (product.productTemplate.quantityPerSheet == 4){
                vc.product = product;
            }
        }
    } else if ([segue.identifier isEqualToString:@"Selected3x3FrameStyleSegue"]) {
        for (OLProduct *product in products){
            if (product.productTemplate.quantityPerSheet == 9){
                vc.product = product;
            }
        }
    } else if ([segue.identifier isEqualToString:@"Selected4x4FrameStyleSegue"]) {
        for (OLProduct *product in products){
            if (product.productTemplate.quantityPerSheet == 16){
                vc.product = product;
            }
        }
    }
        ...
        for (OLProduct *product in products){
            if (product.productTemplate.quantityPerSheet == 1){
                vc.product = product;
            }
        }
    }
